...
class RecipeTestCase(TestCase):
    def setUp(self):
        ...
        self.recipe_b = Recipe.objects.create(
            name="Grilled Chicken Tacos", user=self.user_a
        )
        self.recipe_ingredient_a = RecipeIngredient.objects.create(
            recipe=self.recipe_a, name="Chicken", quantity="1/2", unit="pound"
        )
...
def test_unit_measure_validation(self):
    ...
    ingredient = RecipeIngredient(
        name="New", quantity=10, recipe=self.recipe_a, unit=invalid_unit
    )
